...
def check_file(absolute_path, pieza):
    switcher = {
        ...
        3: "{}/file/EJE.NC".format(absolute_path)
        }
...
def __robotconfig(robot):
	...
	robot.key_torno.teach_routine()
	robot.execute.MO(400, "O")

def __cncconfig(machine):
    print("[TORNO][INFO] PROCESO DE CONFIGURACION INICIADO")
    machine.execute.open_helper()
    machine.execute.open_door()
    machine.execute.open_helper()
    machine.execute.close_door()
    machine.execute.code("G28 U0 W0")
    machine.execute.open_door()
    machine.execute.close_helper()
    machine.execute.close_helper()

def __runstationprocess(files, server_event, machine, robot, pieza):
    machine.execute.open_helper()
    time.sleep(2)
    robot.key_torno.run_movement(1)

    machine.execute.close_helper()
    time.sleep(2)
    robot.key_torno.run_movement(2)
    ...
    robot.key_torno.run_movement(3)
    machine.execute.open_helper()
    time.sleep(2)

    robot.key_torno.run_movement(4)
    time.sleep(1)

    reply = "Proceso Torno Completado"
    try:
